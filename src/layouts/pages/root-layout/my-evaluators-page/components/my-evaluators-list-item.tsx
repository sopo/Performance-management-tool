        ...
        <Avatar>
          <AvatarFallback>
            <p className="text-small text-foreground">
              {lang === "en" ?  peer.peer_display_name_en?.[0] : peer.peer_display_name_ka?.[0] }
            </p>
          </AvatarFallback>
        </Avatar>
        <div className="flex gap-2">
          <p>{lang === "en" ?  peer.peer_display_name_en : peer.peer_display_name_ka}</p>
          <p className="text-muted-foreground">•</p>
          <p className="text-muted-foreground">{lang === "en" ? peer.peer_position_en : peer.peer_position_ka}</p>
          </div>
      </div>
      <Trash2 className="text-muted-foreground w-6 h-6 hover:cursor-pointer" onClick={() =>handleDelete(peerIdToDelete)}/>
      </div>
    )
}
